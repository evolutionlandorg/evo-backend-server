...
import (
	...
	"github.com/evolutionlandorg/evo-backend/pkg/github.com/regcostajr/go-web3/providers/util"
...
func (provider IPCProvider) SendRequest(v interface{}, method string, params interface{}) error {
	...
	if err != nil {
		log.Debug("SendRequest DialUnix error. endpoint=%s. error=%s", provider.endpoint, err)
		return err
	}
	...
	if err := encoder.Encode(bodyString); err != nil {
		log.Debug("SendRequest Encode json error: %s", err)
		return err
	}

	if err := decoder.Decode(v); err != nil {
		log.Debug("SendRequest Decode json error: %s", err)
		return err
	}
